...
class Student extends Admin_Controller
{
    ...
    public function create()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data_insert = array(
                ...
                'student_adhar_no'   => $this->input->post('student_adhar_no'),
                'father_pan_card'   => $this->input->post('father_pan_card'),
                'father_adhar_no'=>$this->input->post('father_adhar_no'),
                'mother_adhar_no'=>$this->input->post('mother_adhar_no'),
                'mother_pan_card'=>$this->input->post('mother_pan_card'),
                'subjects'              => $this->input->post('subjects'),
            );
            if ($this->sch_setting_detail->guardian_occupation) {
                $data_insert['guardian_occupation'] = $this->input->post('guardian_occupation');
            ...
            if ($insert) {
                $code=$data_insert['admission_no'];
                $this->load->library('zend');
                $this->zend->load('Zend/Barcode');
                $imageResource = Zend_Barcode::factory('code128', 'image', array('text' => $code, 'barHeight' => 20, 'setFont' => 6, 'stretchText' => FALSE,'backgroundColor'=>'#EFEFEF'), array())->draw();
               imagejpeg($imageResource, 'barcodes/' . $code . '.png');

                $data_insert['barcode'] = 'barcodes/' . $code . '.png';
    ...
    public function edit($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (isset($father_pan_card)) {

                $data['father_pan_card'] = $this->input->post('father_pan_card');
            }

            if (isset($father_adhar_no)) {

                $data['father_adhar_no'] = $this->input->post('father_adhar_no');
            }
            ...
            if (isset($mother_pan_card)) {

                $data['mother_pan_card'] = $this->input->post('mother_pan_card');
            }

            if (isset($mother_adhar_no)) {

                $data['mother_adhar_no'] = $this->input->post('mother_adhar_no');
            }
            $default_image = array('uploads/student_images/default_female.jpg', 'uploads/student_images/default_male.jpg');
            if (in_array($student['image'], $default_image)) {
                if ($this->input->post('gender') == 'Female') {
             ...
             if (!$this->sch_setting_detail->adm_auto_insert) {
                ...
                $data['admission_no'] = $this->input->post('admission_no');
                $code=$this->input->post('admission_no');
            }else{
                 $code=$this->input->post('admission_no');
            }
    ...
    public function application($id){
        ...
        $data['sch_setting']        = $this->sch_setting_detail;
            ...
            $this->load->view('layout/header', $data);
            $this->load->view('student/studentAplication', $data);
            $this->load->view('layout/footer', $data);
