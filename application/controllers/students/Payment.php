        ...
        if ($this->input->server('REQUEST_METHOD') == "POST") {
            ...
            if (!empty($row_counter)) {
                ...
                $params = array( //payment session
                    ...
                    'paytm_website'             => $pay_method->paytm_website,
                    'invoice'                   => $page,
                    ...
                    'guardian_name'             => $student_record['guardian_name'],
                ...
                if ($pay_method->payment_type == "payu") {
                ...
                } else if ($pay_method->payment_type == "zenda") {
                    if ($pay_method->api_secret_key == "" || $pay_method->api_publishable_key == "" || $pay_method->paytm_website == "") {
                        $this->session->set_flashdata('error', '<div class="alert alert-danger">zenda settings not available</div>');
                        redirect($_SERVER['HTTP_REFERER']);
                    } else {

                        $this->session->set_userdata("params", $params);
                        redirect(base_url("students/zenda"));
                    }
                    ...
                    if ($pay_method->api_secret_key == "" || $pay_method->api_publishable_key == "") {
                        ...
                        $this->session->set_userdata("params", $params);
                        redirect(base_url("students/cashfree"));
                    }
