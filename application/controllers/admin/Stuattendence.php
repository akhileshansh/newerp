...
class Stuattendence extends Admin_Controller {
 ...
 function rfid(){
        ...
        $data['title_list'] = 'Fees Type List';
            ...
            $this->load->view('layout/header', $data);
            $this->load->view('admin/stuattendence/rfid', $data);
            $this->load->view('layout/footer', $data);
    ...
    function rfidsave(){
       ...
       if($admission_no){
        $student_details=$this->student_model->findByAdmission($admission_no);
        if($student_details){
            $time = date('h:i:s a', time());
        $fromtime = date('h:i:s a', strtotime('10:30:00'));
        $totime = date('h:i:s a', strtotime('11:59:59'));
        if (strtotime($time) <= strtotime($fromtime)) {
            $attendence_type_id=1;
        } else if (strtotime($time) <= strtotime($totime)) {
            $attendence_type_id=3;
        } else {
            $attendence_type_id=6;
        }
           $array = array(
         ...
         if($insert_id==true){
         $arr=array('status'=>1,'message'=>$this->lang->line('attendance_success'));
         }else{
             $arr=array('status'=>2,'message'=>$this->lang->line('attendance_already'));
         }
      
        
        }else{
              $arr=array('status'=>0,'message'=>$this->lang->line('not_found'));
        }
       }else{
         $arr=array('status'=>0,'message'=>$this->lang->line('not_found'));
       }
