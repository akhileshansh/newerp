...
class Paymentsettings extends Admin_Controller {
  ...
  public function pesapal(){
    ...
    public function setting() { 
        ...
        $this->form_validation->set_rules('cashfree_merchantid', $this->lang->line('key'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('cashfree_merchantkey', $this->lang->line('key'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('cashfree_website', $this->lang->line('key'), 'trim|required|xss_clean');
        if ($this->form_validation->run()) {

            $data = array(
                'api_secret_key' => $this->input->post('cashfree_merchantkey'),
                'api_publishable_key' => $this->input->post('cashfree_merchantid'),
                'paytm_website' => strtoupper($this->input->post('cashfree_website')),
            ...
            $data = array(
                'cashfree_merchantkey' => form_error('cashfree_merchantkey'),
                'cashfree_merchantid' => form_error('cashfree_merchantid'),
                'cashfree_website' => form_error('cashfree_website'),
            );
            echo json_encode(array('st' => 1, 'msg' => $data));
        }
    }
