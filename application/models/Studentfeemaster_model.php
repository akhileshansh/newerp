...
class Studentfeemaster_model extends MY_Model
{
    ...
    public function searchAssignFeeByClassSectionold($class_id = null, $section_id = null, $fee_session_group_id = null, $category = null, $gender = null, $rte = null)
    {
        ...
        public function searchAssignFeeByClassSection($class_id = null, $section_id = null, $fee_session_group_id = null, $category = null, $gender = null, $rte = null)
    {
        ...
        . " `students`.`city`, `students`.`pincode`, `students`.`religion`, `students`.`dob`, "
        . "`students`.`current_address`, `students`.`permanent_address`,"
        . " IFNULL(students.vehroute_id, 0) as `vehroute_id`, "
        . " IFNULL(vehicle_routes.route_id, 0) as `route_id`, "
        . " IFNULL(vehicle_routes.vehicle_id, 0) as `vehicle_id`, "
        . " IFNULL(transport_route.route_title,' ') as `route_title`, "
        . " IFNULL(students.category_id, 0) as `category_id`,"
        . " IFNULL(categories.category, '') as `category`,"
        . " `students`.`adhar_no`, `students`.`samagra_id`,"
        . " `students`.`bank_account_no`, `students`.`bank_name`, `students`.`ifsc_code`,"
        ...
        . " `students`.`gender` FROM `students` JOIN `student_session` "
        . "ON `student_session`.`student_id` = `students`.`id` JOIN `classes` "
        . "ON `student_session`.`class_id` = `classes`.`id` LEFT JOIN `vehicle_routes` "
        . " ON `students`.`vehroute_id` = `vehicle_routes`.`id` LEFT JOIN `transport_route` "
        . " ON `vehicle_routes`.`route_id` = `transport_route`.`id` JOIN `sections` "
        . "ON `sections`.`id` = `student_session`.`section_id` LEFT JOIN `categories` "
    ...
    public function studentDepositByFeeGroupFeeTypeArray($student_session_id, $fee_type_array)
    {
     ...
     public function zendagetStudentFees($student_session_id)
    {
        $sql    = "SELECT `student_fees_master`.*,fee_groups.name FROM `student_fees_master` INNER JOIN fee_session_groups on student_fees_master.fee_session_group_id=fee_session_groups.id INNER JOIN fee_groups on fee_groups.id=fee_session_groups.fee_groups_id  WHERE `student_session_id` = " . $student_session_id . " ORDER BY `student_fees_master`.`id`";
        $query  = $this->db->query($sql);
        $result = $query->result();
        if (!empty($result)) {
            foreach ($result as $result_key => $result_value) {
                ...
                $fee_session_group_id   = $result_value->fee_session_group_id;
                $student_fees_master_id = $result_value->id;
                $result_value->fees     = $this->zendagetDueFeeByFeeSessionGroup($fee_session_group_id, $student_fees_master_id);
    ...
    /*  public function zendagetStudentFeesAnual($student_session_id)
    {
        $sql    = "SELECT `student_fees_master`.*,fee_groups.name FROM `student_fees_master` INNER JOIN fee_session_groups on student_fees_master.fee_session_group_id=fee_session_groups.id INNER JOIN fee_groups on fee_groups.id=fee_session_groups.fee_groups_id  WHERE `student_session_id` = " . $student_session_id . " ORDER BY student_fees_master.id desc";
        $query  = $this->db->query($sql);
        $result = $query->result();
        if (!empty($result)) {
            foreach ($result as $result_key => $result_value) {
     ...
     public function zendagetDueFeeByFeeSessionGroup($fee_session_groups_id, $student_fees_master_id)
    {
        $sql = "SELECT student_fees_master.*,fee_groups_feetype.id as `fee_groups_feetype_id`,fee_groups_feetype.amount,fee_groups_feetype.due_date,fee_groups_feetype.fine_amount,fee_groups_feetype.fee_groups_id,fee_groups.name,fee_groups_feetype.feetype_id,feetype.code,feetype.type, IFNULL(student_fees_deposite.id,0) as `student_fees_deposite_id`, IFNULL(student_fees_deposite.amount_detail,0) as `amount_detail` FROM `student_fees_master` INNER JOIN fee_session_groups on fee_session_groups.id = student_fees_master.fee_session_group_id INNER JOIN fee_groups_feetype on  fee_groups_feetype.fee_session_group_id = fee_session_groups.id  INNER JOIN fee_groups on fee_groups.id=fee_groups_feetype.fee_groups_id INNER JOIN feetype on feetype.id=fee_groups_feetype.feetype_id LEFT JOIN student_fees_deposite on student_fees_deposite.student_fees_master_id=student_fees_master.id and student_fees_deposite.fee_groups_feetype_id=fee_groups_feetype.id WHERE student_fees_master.fee_session_group_id =" . $fee_session_groups_id . " and student_fees_master.id=" . $student_fees_master_id . " order by fee_groups_feetype.due_date ASC";
    ...
    function update_deposit_collections($data){
        if (!empty($data)) {
            $collected_fees = array();
 
                foreach($data as $v){
                    $uin['transactionNo']=$v['transactionNo'];
                    $uin['invoiceAmount']=$v['invoiceAmount'];
                    $uin['advanceAmount']=$v['advanceAmount'];
                    $uin['totalAmount']=$v['totalAmount'];
                    $uin['paymentStatus']=$v['paymentStatus'];
                    $uin['orderId']=$v['orderId'];
                    $uin['totalTaxAmt']=$v['totalTaxAmt'];
                    $uin['studentName']=$v['studentName'];
                    $uin['registerNo']=$v['registerNo'];
                    $uin['receiptNo']=$v['receiptNo'];
                    $uin['onlinePaymentAppTransactionId']=$v['onlinePaymentAppTransactionId'];
              
                    $this->db->where('transactionNo', $v['transactionNo']);
                    $q = $this->db->get('paymentdetails');
                    if ($q->num_rows() > 0) {
                   
                    $collected_fees[] = array('transactionNo' => $v['transactionNo'], 'status' => 'failure','message'=>'Transaction already exists');
                    } 
                    else {

                    $this->db->insert('paymentdetails', $uin);
                    $inserted_id      = $this->db->insert_id();
                    if(!empty($v['invoiceList'])){
                        foreach($v['invoiceList'] as $ins){
                            $inv['invoiceNo']=$ins['invoiceNo'];
                            $inv['invoiceDueAmt']=$ins['invoiceDueAmt'];
                            $inv['payment_details_id']=$inserted_id;
                            $this->db->where('invoiceNo', $ins['invoiceNo']);
                            $q1 = $this->db->get('invoices');
                            if ($q1->num_rows() > 0) {
                                $this->db->update('invoices', $inv);
                            }else{

                            //   $studentsession=$this->studentfeemaster_model->zendagetStudentFeesinvoice($ins['invoiceNo']);
                            //       if(!empty($studentsession)){
                            //     $collected_array[] = array();
                            //       foreach ($studentsession as $fee_key => $fee) {
                                     
                            //             if($ins['invoiceNo']==$fee->invoiceNo){
                    ...
                    $collected_fees[]= array('transactionNo' => $v['transactionNo'], 'status' => 'success','message'=>'Success');    
                }
            } 
     ...
     public function zendagetStudentFeesinvoice($student_session_id)
    {
        $sql    = "SELECT `student_fees_master`.*,fee_groups.name FROM `student_fees_master` INNER JOIN fee_session_groups on student_fees_master.fee_session_group_id=fee_session_groups.id INNER JOIN fee_groups on fee_groups.id=fee_session_groups.fee_groups_id  WHERE `invoiceNo` = '" . $student_session_id . "' ORDER BY `student_fees_master`.`id`";
        $query  = $this->db->query($sql);
        $result = $query->result();
        if (!empty($result)) {
            foreach ($result as $result_key => $result_value) {
    ...
    public function update_loan($data){
        if($data){
            $collected=array();
            $this->db->where('loanApplicationNo', $data['loanApplicationNo']);
            $this->db->where('registerNo', $data['registerNo']);
            $this->db->where('academicYear', $data['academicYear']);
            $q = $this->db->get('loan');
            if ($q->num_rows() > 0) {
                   
                $collected =array('HttpStatus'=> 409,'message'=>'loanApplicationNo already exists');
            }else{
                  $this->db->insert('loan', $data);
                    $inserted_id      = $this->db->insert_id();
                    if($inserted_id){
                        $collected=array('HttpStatus'=>200,'message'=>'Success');
                    }
    ...
    public function getzendapayments($value='')
    {
        ...
        if (!empty($result)) {
            foreach ($result as $result_key => $result_value) {
                ...
                $result_value->student_details=$this->findByAdmission($result_value->registerNo);
                $result_value->invoices   = $this->zendainvoicepayment($result_value->id);
    ...
    function zendainvoicepayment($id){
      if($id){
        $this->db->select('invoiceNo,invoiceDueAmt');
